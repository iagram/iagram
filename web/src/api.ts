import type { Catalog, Document, DriftResult, Job, PlanResult, Validation, TemplateItem, LayoutOptions } from './types'
...
export const api = {
  ...
  layout: (doc: Document, options: LayoutOptions, root?: string) =>
    fetch('/api/layout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ document: doc, options, root }) }).then((r) => json<{ document: Document }>(r)),
