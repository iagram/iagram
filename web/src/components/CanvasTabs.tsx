...
export function CanvasTabs() {
  ...
  return (
    <div className="canvas-tabs" role="tablist">
      {providers.map((p) => (
        <button key={p} role="tab" aria-selected={p === active} className={p === active ? 'active' : ''} onClick={() => setActive(p)}>
          {LABEL[p] ?? p}
          <span className="count">{counts[p] ?? 0}</span>
        </button>
      ))}
      <span className="hint">One canvas per provider; Plan and Apply cover the whole file.</span>
    </div>
  )
}
