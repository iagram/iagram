...
| Area | What it does |
|---|---|
| **Cloud tabs** (above the palette) | One tab per provider; the mirror toggle sits next to Save. In **mirror** mode (default) the tab you draw on is the source and the other tabs show its live equivalent through the equivalence table, dropped elements listed in a banner; editing a mirrored tab makes that provider the source. Turn mirroring off for independent canvases in one file. Plan/Apply run the source. |
...
| **Menubar and toolbar** | File (home, new, open, import from Terraform configuration or state, save, download, export Terraform / PNG / SVG), Edit (undo, redo, cut/copy/paste, z-order, select all), View (zoom, labels, legend, snap, theme), Arrange (horizontal flow, vertical flow, grid, circle; applies to the whole diagram or to the selected container), Infrastructure (plan, apply, drift, destroy, log), Help (shortcuts, docs, about); quick buttons for undo/redo, zoom, labels, theme, Save, Check drift, Plan, Apply. |
