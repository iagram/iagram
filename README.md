...
| **Menubar and toolbar** | File (home, new, open, import from Terraform configuration or state, save, download, export Terraform / PNG / SVG), Edit (undo, redo, cut/copy/paste, z-order, select all), View (zoom, labels, legend, snap, theme), Arrange (horizontal and vertical flow, horizontal and vertical tree, radial tree, organic, circle, org chart, grid; each opens a dialog with node and rank spacing, direction, container resizing and origin options; applies to the whole diagram or to the selected container; the flow keeps connected elements level so arrows run straight, and arrows leave and enter by the sides facing each other unless you pick the anchors), Infrastructure (plan, apply, drift, destroy, log), Help (shortcuts, docs, about); quick buttons for undo/redo, zoom, labels, theme, Save, Check drift, Plan, Apply. |
