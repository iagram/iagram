...
The front page of the editor is a gallery of more than 550 official reference architectures from AWS, Google Cloud and Azure (networking and hybrid, web, serverless, containers, data and streaming, ML and GenAI, security and landing zones, resilience and DR, IoT and media, DevOps, industry solutions), redrawn as `.iad` diagrams that validate and generate Terraform. Every template is rendered, validated and generated in CI. Filter by cloud, category or text, read the source page, and open one in the editor with everything pre-configured. Cards show the vendor's original diagram when one exists (252 of them, bundled and downscaled, checked one by one against the template and credited through each card's Official reference link and in `templates/NOTICE.md`), and a generated miniature otherwise. A card always opens a **copy**: the reference architectures ship read-only inside the binary and never change; Save writes the copy to your own `.iad`. "+ New IaD" starts a blank canvas, and the iagram logo brings you back home. The gallery is the home page at `/`; the canvas lives at `/editor`, so both can be bookmarked and the browser back button moves between them. It opens from File > Home too; "Open my diagram" takes you to the current file.

Each template is a compact spec under `templates/<provider>/<slug>/template.yaml` (see `docs/spec/templates.md`). Fidelity to the vendor drawing is explicit, not heuristic: every template with a bundled picture carries, per element, the grid cell it occupies in the reference (`at: [row, col]`, with spans), read off the vendor image, and the layout reproduces that arrangement (edge services in a left column, Availability Zones as rows with subnet columns, a load balancer between them, hub in the centre with spokes around); templates without cells fall back to the flow heuristic. Content follows the reference too: every template with a trustworthy picture had its research-catalogue entry rewritten to the elements, actors and numbered flows the vendor draws (route tables, QuickSight, CloudWatch, Microsoft Entra ID as an identity-provider actor, and so on) and was regenerated by the converter, which keeps the cells. `docs/fidelity-reconciliation.md` reports how near each template is to its reference and what the model cannot express. `iagram templates list` prints them, `iagram templates build DIR` writes their `.iad` files, and `iagram init --template aws/tgw-hub-and-spoke` starts a project from one. Placeholders such as `changeme`, the all-zero subscription id or `my-project-123456` mark the values you must set.

## The editor

| Area | What it does |
|---|---|
| **Shell** | The canvas fills the window under a slim docked header (iagram logo, menu bar, file name, mirror toggle, status, Save, Check drift, Plan, Apply). Everything else floats over the canvas: a left rail toggles the palette and settings panel, switches between select (V) and hand (H) pointer modes, inserts notes, groups and users, undoes/redoes and opens Arrange; the palette and settings panel are floating cards. Clicking the empty canvas puts the settings panel away and selecting an element brings it back; pin it (top right of the panel) to keep it open, or collapse it with the arrow. Zoom controls sit bottom right, problems bottom left. |
| **Cloud tabs** (top of the palette) | One tab per provider; the mirror toggle sits next to Save. In **mirror** mode (default) the tab you draw on is the source and the other tabs show its live equivalent through the equivalence table, dropped elements listed in a banner; editing a mirrored tab makes that provider the source. Turn mirroring off for independent canvases in one file. Plan/Apply run the source. |
...
| **Canvas** | Elements drawn as bare service icons with their name underneath, as in the official diagrams (View > Elements as cards restores boxes); nested containers drawn with the official colour grammar (account → region → Availability Zone → VPC → subnet, organization → OU), generic groups, corporate data centers, actors (users, clients, internet, devices, SaaS), notes, Auto Scaling bands stretched over the subnets they cover (also when a template or the layout places them), typed arrows with labels, numbered step callouts drawn as badges (the text shows on hover, selection and in the legend), styles (both ways, dashed, inactive), link lines for peering / attachments / VPN, plan and drift overlays, legend. Shift-drag to select several, ⌘-click to add. Drag an element into another container to move it there (refused with a message if the catalog forbids it); right-click for cut / copy / paste / z-order. |
| **Settings panel** (right) | Name, **Presentation** for containers (box with its elements inside, or icon: collapsed to the service icon with everything inside hidden and arrows re-attached, to keep a complex diagram readable; Kubernetes clusters are an icon until something is placed in them), **Annotation** (caption, step number), properties in sections (`Compute`, `Networking`, `Advanced`…), **Attachments** (add and configure the non-graphical resources that apply to this element), **Components** for cluster boxes, validation messages, planned resource changes, drift details, live outputs after apply (click to copy), delete. Click an arrow for the connection inspector: label, step, and visual controls for arrow heads, connector shape (orthogonal, straight, curved), line style, width and colour, plus for link lines the Terraform resource and its attributes. With nothing selected: the walkthrough steps editor and the legend toggle. |
| **Menubar and toolbar** | File (home, new, open, import from Terraform configuration or state, save, download, export Terraform / PNG / SVG), Edit (undo, redo, cut/copy/paste, z-order, select all), View (zoom, labels, legend, snap, theme), Arrange (horizontal and vertical flow, horizontal and vertical tree, radial tree, organic, circle, org chart, grid; each opens a dialog with node and rank spacing, direction, container resizing and origin options; applies to the whole diagram or to the selected container; the flow keeps connected elements level so arrows run straight, arrows leave and enter by the sides facing each other unless you pick the anchors, and orthogonal connectors are routed around the icons in their way instead of through them), Infrastructure (plan, apply, drift, destroy, log), Help (shortcuts, docs, about); quick buttons for undo/redo, zoom, labels, theme, Save, Check drift, Plan, Apply. |
| **Log drawer** | Streams `tofu` output for plan/apply/drift; lists orphaned state that a plan would destroy. |

Shortcuts: ⌘S save · ⌘Z / ⌘⇧Z undo, redo · ⌘X / ⌘C / ⌘V / ⌘D cut, copy, paste, duplicate · ⌘] / ⌘[ (⇧ for front/back) z-order · ⌘A select all · Esc deselect · ⌫ delete · `?` shortcut list · double-click an element to focus its settings. Deep links: `/editor`, `?theme=dark`, `?provider=gcp`, `?legend=1`, `#select=<node id>`.
