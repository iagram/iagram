...
| `edges[].style` | Optional drawing: `direction` `one` (default) / `both` / `none` (plain association line), `dash` `solid` / `dashed` / `dotted`, `color` (CSS), `width` (px), `curve` `step` / `straight` / `bezier`, `inactive` (greyed standby path). The catalog rule of the kind provides defaults. Informational. |
